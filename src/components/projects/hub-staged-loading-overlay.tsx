...
type HubStagedLoadingOverlayProps = {
  visible: boolean;
...
function resolveStageIndex(
  ...
  return Math.min(Math.floor((Date.now() - startedAt) / stageMs), stageCount - 1);
}

export function HubStagedLoadingOverlay({
  visible,
  stages,
  stageMs,
  subtitle,
  startedAt,
}: HubStagedLoadingOverlayProps) {
  ...
  useEffect(() => {
    ...
    setStageIndex(resolveStageIndex(startedAt, stageMs, stages.length));
  ...
  }, [visible, startedAt, stageMs, stages.length]);

  const activeLabel = stages[stageIndex] ?? stages[0];
