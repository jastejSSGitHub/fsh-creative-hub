...
function ProjectCardSkeleton({ card }: { card: ProjectCardSnapshot }) {
  return (
    <article className="flex flex-col overflow-hidden rounded-xl border border-hub-espresso/10 bg-white">
      <div className="relative aspect-[16/10] overflow-hidden bg-[linear-gradient(135deg,#faf8f3_0%,#f3efe6_55%,#ffffff_100%)]">
        {card.hasCover ? (
          <SkeletonBone className="size-full rounded-none bg-hub-espresso/[0.05]" />
        ) : (
      ...
      <div className="space-y-1 border-t border-hub-espresso/8 bg-hub-espresso/[0.03] p-3">
        <div className="flex items-start gap-2">
          <SkeletonBone className="mt-0.5 size-5 shrink-0 rounded bg-hub-espresso/[0.1]" />
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="inline-flex min-h-10 w-full items-center justify-center rounded-full border border-hub-espresso/12 bg-white px-4 sm:w-auto">
      ...
      <div className="-mx-3 overflow-x-auto px-3 sm:mx-0 sm:px-0">
        <div className="flex min-w-max items-center gap-1 border-b border-hub-espresso/10 pb-1">
          ...
          <div className="rounded-md px-3 py-1.5">
            <SkeletonBone className="h-4 w-12 bg-hub-espresso/[0.05]" />
          </div>
        </div>
      </div>
