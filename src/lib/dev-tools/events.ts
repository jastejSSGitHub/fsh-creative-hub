...
export const DEV_TOOLS_MOCK_COLLABORATION_CHANGED =
  "fsh-dev-tools-mock-collaboration-changed";
...
export function dispatchDevToolsMockCollaborationChanged(enabled: boolean): void {
  if (typeof window === "undefined") return;
  window.dispatchEvent(
    new CustomEvent(DEV_TOOLS_MOCK_COLLABORATION_CHANGED, {
      detail: { enabled },
    }),
  );
}
